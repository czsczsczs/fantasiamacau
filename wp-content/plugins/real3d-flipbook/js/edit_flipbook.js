...
(function($) {
    ...
    $(document).ready(function() {
        ...
        addOptionGeneral( 
            ...
            "text", 
            "PDF page size (small)", 
            'height of rendered PDF pages in px (small size)'
        );

        addOptionGeneral( 
            "pageTextureSizeTreshold",  
            "text", 
            "PDF page size treshold", 
            'above treshold it will render PDF in default (full) page size, below treshold it will render PDF in small page size'
        );
        ...
        function convertPdfPages(){

            $(".pdf-to-jpg-info").text("Converting page "+creatingPage + ' / '+pdfDocument._pdfInfo.numPages)

            var pageSize = Number(getOptionValue('pageTextureSize'))

            convertPdfPageToImage(creatingPage, "page"+creatingPage, pageSize, function(pageUrl){

                var d = new Date()
                var pageUrl = pageUrl + "?" + d.getTime();

                var thumbSize = Number(getOptionValue('thumbSize'))

                convertPdfPageToImage(creatingPage, "thumb"+creatingPage, thumbSize, function(thumbUrl){
                    
                    thumbUrl = thumbUrl + "?" + d.getTime();
                    var pagesContainer = $("#pages-container");
                    var pageItem = createPageHtml($(".page").length + 1, '', pageUrl, thumbUrl);
                ...
                generateLightboxThumbnail()
                    ...
                    if(creatingPage < pdfDocument._pdfInfo.numPages){

                        creatingPage++;
                        convertPdfPages()
                    }else{
                        $(".pdf-to-jpg-info").text("")
                    }
                })
            })
        ...
        function addListeners() {
            ...
            $('.pdf-to-jpg-button').click(function(e) {
                e.preventDefault();

                //if ($('.page').length == 0 || confirm('All current pages will be lost. Are you sure?')) {

                    selectPdfFile(pdfToJpg, true)
        ...
        $('#edit-page-html-content').bind('change keyup paste', function() {
            ...
            // setHtmlContent(dataId, ($(this).val()))
